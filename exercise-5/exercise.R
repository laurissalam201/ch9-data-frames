...
most_popular_in_year <- function(this_year){
  names_year <- names[names$year == this_year, ]
...
number_in_million <- function(this_name, this_year){
  pop <- names[names$year == this_year & names$name == this_name, 'prop']
  pop*1000000
}
...
number_in_million('Laurissa', 1998)
